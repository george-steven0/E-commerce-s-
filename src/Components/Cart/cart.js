...
const Cart = (props) => {
    ...
    const listCartItem = cart?.cart?.map( (item,index)=>{
        return (
            <div key={index} className="grid grid-cols-4 mb-2 border-t-[1px] border-[#c5c1c18e] rounded-md px-3 py-2 pt-2 transition-all duration-300 hover:bg-[#e9732528]">
                <CartItem  
                    src={item.images[0]} 
                    qun={1} 
                    price={item.price} 
                    alt={item.description} 
                    filterOut={()=>removeCartItem(item)}  
                />
            </div>
        )
    } )
    ...
    let total = 0
    let totalCartPrice = cart?.cart?.map( (item)=>{
    ...
    return ( 
        <article className="cart w-full h-screen">
            <section className="absolute top-4 left-4 w-8 h-8 rounded-[50%] bg-white cursor-pointer z-10 text-center leading-8" onClick={props.toggleCart}><FontAwesomeIcon className="text-[#E97425] text-xl" icon ={faTimes}  /></section>
            ...
            <section className="overlay absolute top-0 bottom-0 left-0 right-0 w-full h-full bg-[#000000ba] px-8">
                <div className="cartItemsWrapper p-4 bg-[#f4f4f4df] mt-[30%] rounded-md ">
                    ...
                    <div className="title flex justify-between align-middle items-center">
                        ...
                        <div className="capitalize font-normal text-base "><span className="text-[#6b6b6b] font-bold">{cart?.length}</span> items</div>
                    </div>
